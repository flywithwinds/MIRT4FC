...
######## Generate a simulated dataset
Y <- data.sim (item.par, theta, BID, blocksize = 3, res = 'rank')
# Fixed item indices and fixed item parameters
fixed.blocks <- c(1:6)
idx <- as.vector(sapply(fixed.blocks, function(j) (3*(j-1)+1):(3*j)))
fa <- matrix(item.par$a[idx], nrow = 3, ncol = length(fixed.blocks))
fd <- matrix(item.par$d[idx], nrow = 3, ncol = length(fixed.blocks))
# Item parameter estimation
fit.ext <- iStEM(Y, BID, maxitr = 30, blocksize = 3, res = 'rank', fix.sigma = TRUE,
