...
kernel_2PL_2 <- function(Y,BID,a,d,sigm,theta,B=20,J,D,N,cor.matrix=FALSE,positive,fix.sigma=FALSE,blocksize=2,hessian=TRUE,fixed.blocks=NULL){
...
objfun_2PL_2 <- function(x,thetaj,yj,blocksize=2){
  ...
  v <- -1*P.Yj_2PL_2(aj=(x[1:blocksize]),dj=c(x[(blocksize+1):IN],-x[(blocksize+1):IN]),thetaj,yj)
...
P.Yj_2PL_2 <- function(aj,#vector of length 2
  ...
  lin <- aj*t(thetaj)-dj # 2 x N; linear predictor
  # numerically stable softmax: subtract column max to avoid exp overflow -> Inf/NaN
  mx <- matrix(apply(lin,2,max),nrow = 2,ncol = ncol(lin),byrow = TRUE)
  z <- exp(lin-mx) # 2 x N, all rows in [0,1]
  ...
  for(b in seq_len(B)){
    # sampling theta - Equation 4
      ...
      if(!is.null(fixed.blocks)){
        opt[, fixed.blocks] <- rbind(a[, fixed.blocks, drop=FALSE], d[1, fixed.blocks, drop=FALSE])
      }
