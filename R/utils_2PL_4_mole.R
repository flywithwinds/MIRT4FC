...
kernel_2PL_4_mole <- function(Y,BID,a,d,sigm,theta,B=20,J,D,N,cor.matrix=FALSE,positive,fix.sigma=FALSE,hessian=TRUE,fixed.blocks=NULL){
...
objfun_2PL_4_mole <- function(x,thetaj,yj){
  v <- -1*P.Yj_2PL_4_mole(aj=(x[1:4]),dj=c(x[5:7],-sum(x[5:7])),thetaj,yj)
