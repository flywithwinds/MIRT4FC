...
kernel_2PL_3_pick <- function(Y,BID,a,d,sigm,theta,B=20,J,D,N,cor.matrix=FALSE,positive,fix.sigma=FALSE,blocksize=3,hessian=TRUE,fixed.blocks=NULL){
...
objfun_2PL_3_pick <- function(x,thetaj,yj,blocksize=3){
  IN <- 2*blocksize-1 # Number of item parameter
  v <- -1*P.Yj_2PL_3_pick(aj=(x[1:blocksize]),dj=c(x[(blocksize+1):IN],-1*sum(x[(blocksize+1):IN])),thetaj,yj)
