...
iStEM_2PL_3_pick <- function(Y,BID,positive=rep(TRUE,nrow(BID)),M=10,B=20,SE='Louis',blocksize=3,
  ...
  # 若指定了固定块的外部初始值，则用其覆盖固定块的参数作为固定值
  if(!is.null(fixed.blocks) && !is.null(fixed.a)){
    a[, fixed.blocks] <- matrix(fixed.a, nrow=blocksize)
  }
  if(!is.null(fixed.blocks) && !is.null(fixed.d)){
    d[, fixed.blocks] <- matrix(fixed.d, nrow=blocksize)
  }
  #initial theta
  if(is.null(theta)){
  ...
  x <- kernel_2PL_3_pick(Y=Y,BID=BID,a=a,d=d,sigm=sigm,theta=theta,B=B,J=J,D=D,N=N,cor.matrix = TRUE,positive=positive,fix.sigma=fix.sigma,hessian = TRUE,blocksize=blocksize,fixed.blocks=fixed.blocks)
  a <- x$a
  d <- x$d
  sigm <- x$sigm
  theta <- x$theta
  for(m in 1:M){
    ...
    cat("\n  # of batch = ",m)
    x <- kernel_2PL_3_pick(Y=Y,BID=BID,a=a,d=d,sigm=sigm,theta=theta,B=B,J=J,D=D,N=N,cor.matrix = (m<M/2),positive=positive,fix.sigma=fix.sigma,hessian = TRUE,blocksize=blocksize,fixed.blocks=fixed.blocks)
    plist[[m]] <- x$pmatrix
...
d.hat <- batch.var(plist,n=m)
 for(i in 1:5){ # 5 iterations
    ...
    x <- kernel_2PL_3_pick(Y=Y,BID=BID,a=a,d=d,sigm=sigm,theta=theta,B=B,J=J,D=D,N=N,positive=positive,fix.sigma=fix.sigma,hessian = TRUE,blocksize=blocksize,fixed.blocks=fixed.blocks)
    a <- x$a
    d <- x$d
    sigm <- x$sigm
    #print(sigm)
