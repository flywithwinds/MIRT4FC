...
Pj_2PL_4_pick <- function(j,theta,aj=NULL,dj=NULL,BID=NULL,item.par=NULL){
  ...
  lin <- aj*t(thetaj)-dj # 4 x N; linear predictor
  # numerically stable softmax: subtract column max to avoid exp overflow -> Inf/NaN
  mx <- matrix(apply(lin,2,max),nrow = 4,ncol = ncol(lin),byrow = TRUE)
  z <- exp(lin-mx) # 4 x N, all rows in [0,1]
...
kernel_2PL_4_pick <- function(Y,BID,a,d,sigm,theta,B=20,J,D,N,cor.matrix=FALSE,positive,fix.sigma=FALSE,hessian=TRUE,fixed.blocks=NULL){
...
objfun_2PL_4_pick <- function(x,thetaj,yj){
  v <- -1*P.Yj_2PL_4_pick(aj=(x[1:4]),dj=c(x[5:7],-sum(x[5:7])),thetaj,yj)
...
logLi_2PL_4_pick <- function(thetai, #vector of length D  ability for person i
  ...
  for(b in seq_len(B)){
    ...
    if(!is.null(fixed.blocks)){
      opt[, fixed.blocks] <- rbind(a[, fixed.blocks, drop=FALSE], d[1:3, fixed.blocks, drop=FALSE])
    }
    hess=diag(rep(0,J*7))
    if(hessian==TRUE){
