...
kernel_2PL_4_rank <- function(Y,BID,a,d,sigm,theta,B=20,J,D,N,cor.matrix=FALSE,positive,fix.sigma=FALSE,hessian=FALSE,fixed.blocks=NULL){
...
objfun_2PL_4_rank <- function(x,thetaj,yj){
  v <- -1*P.Yj_2PL_4_rank(aj=(x[1:4]),dj=c(x[5:7],-sum(x[5:7])),thetaj,yj)
...
P.Yi_2PL_4_rank <- function(a,#matrix 4 x J
  ...
  response.pattern <- matrix(c(1,2,3,4,
                               ...
                               4,3,1,2,
                               4,3,2,1),ncol = 4,byrow = TRUE)
  lin <- a*theta-d # 3 x N; linear predictor
  # numerically stable softmax: subtract column max to avoid exp overflow -> Inf/NaN
  mx <- matrix(apply(lin,2,max),nrow = 4,ncol = ncol(lin),byrow = TRUE)
...
logL_2PL_4_rank <- function(theta, #vector of length D  ability for person i
  ...
  response.pattern <- matrix(c(1,2,3,4,
                               ...
                               4,3,1,2,
                               4,3,2,1),ncol = 4,byrow = TRUE)
  lin <- a*thetai-d # 3 x N; linear predictor
  # numerically stable softmax: subtract column max to avoid exp overflow -> Inf/NaN
  mx <- matrix(apply(lin,2,max),nrow = 4,ncol = ncol(lin),byrow = TRUE)
  z <- exp(lin-mx) # 3 x N, all rows in [0,1]
  sumz <- matrix(colSums(z)[col(z)],nrow = 4)
